...
要实现ICE通讯，需要双方端点都支持ICE。但是在一些时候，某写端点具有公网地址时，于是它可以和任何通讯点进行通讯。为了让这类端点更容易的实现ICE通讯，
ICE定义了一种简单的实现：简化实现。简化实现不再收集候选地址，它只包含主机候选地址。精简节点不在使用连接检查或状态机机制，这样它需要反馈连接检查。
当一个精简节点连接一个完全节点时，完全节点作为被控制端点而精简节点作为控制端点。于是他们之间相互连接时，不发生任何连接检查。

附录A有一个关于简化实现时的实现讨论。

要注意简化实现只是作为完全实现的一个进阶版本。对于那些具有外网地址的通讯端点，完全实现如果可行是更可取的。
